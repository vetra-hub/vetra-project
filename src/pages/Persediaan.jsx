...
import persediaanData from "../persediaan.json"; // Mengimpor data dari file JSON

const Persediaan = () => {
  const [persediaan, setPersediaan] = useState(persediaanData);
  ...
  return (
    <div className="flex flex-col flex-1 w-full h-full overflow-auto bg-gray-50">
          ...
          <tbody className="bg-white">
            {persediaan.map((item) => (
              <tr key={item.no} className="border-t hover:bg-gray-50">
                ...
                <td className="px-6 py-4">{formatCurrency(item.hargaPokokMin)} - {formatCurrency(item.hargaPokokMax)}</td>
                <td className="px-6 py-4">{formatCurrency(item.hargaJualMin)} - {formatCurrency(item.hargaJualMax)}</td>
                <td className="px-6 py-4">{item.minMarkup}% - {item.maxMarkup}%</td>
                ...
                <td className="px-6 py-4 text-center">
                  <div className="flex justify-center space-x-3">
                    ...
                    <button className="bg-blue-500 text-white rounded-lg px-4 py-2 text-xs hover:bg-blue-600 focus:outline-none transition-all flex items-center">
                      <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" aria-hidden="true">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 3l4 4m0 0l-4 4m4-4H6m0 0l4 4m0 0l-4 4"></path>
                      </svg>
                      Edit
                    </button>
                    ...
                    <button className="bg-yellow-500 text-white rounded-lg px-4 py-2 text-xs hover:bg-yellow-600 focus:outline-none transition-all flex items-center">
                      <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" aria-hidden="true">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 3h4m4 0h4M12 12v7m0 0H5.5A1.5 1.5 0 014 17.5V7.5A1.5 1.5 0 015.5 6h13A1.5 1.5 0 0120 7.5V17a1.5 1.5 0 01-1.5 1.5H12z"></path>
                      </svg>
                      Beli
                    </button>
                  </div>
      ...
      <div className="bg-white rounded-lg shadow-lg p-6 mt-8">
        <h2 className="text-xl font-semibold mb-6 text-gray-700">Kondisi Produk</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
          <div className="flex flex-col space-y-4">
            <p>Produk Dijual: <span className="font-semibold text-gray-800">8919 produk</span></p>
            <p>Produk Berpotensi Rugi: <span className="font-semibold text-gray-800">738 produk</span></p>
          </div>
          <div className="flex flex-col space-y-4">
            <p>Produk Tidak Dijual: <span className="font-semibold text-gray-800">319 produk</span></p>
            <p>Produk Stok Negatif: <span className="font-semibold text-gray-800">37 produk</span></p>
          </div>
        </div>
      </div>
    </div>
