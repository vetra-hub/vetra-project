...
export const obatresep = {
  async fetchObatResep() {
    const response = await axios.get(
      API_URL + "?select=id,created_at,keterangan,gambar",
      { headers }
    );
    return response.data;
  },

  async createObatResep(data) {
    const response = await axios.post(API_URL, data, { headers });
    return response.data;
  },

  async updateObatResep(id, data) {
